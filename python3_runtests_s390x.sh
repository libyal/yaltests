#!/bin/bash
#
# Script to run Python tests on s390x
...
RUN apt-get -y install autoconf git locales python3
...
docker run -t -v "${PWD}:/${LIBRARY_NAME}:z" ${TAG} sh -c "cd ${LIBRARY_NAME} && python3 ./run_tests.py"
