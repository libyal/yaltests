"""Tests for the coreutils stat output parser."""

import unittest

# Note stat we cannot use stat given Python already has a module named stat
from coreutils import custat

from tests import test_lib


class StatOutputParserTest(test_lib.BaseTestCase):
    """Tests for the coreutils stat output parser."""

    # TODO: add tests for _parse_date_time
    # TODO: add tests for _parse_hexadecimal

    def test_parse_with_ext2_inode256(self):
        """Tests the parse function with ext2 coreutils stat output."""
        test_file = self._get_test_file_path(["mke2fs-1.47.0", "stat.ext2.22.txt"])
        self._skip_if_path_not_exists(test_file)

        with open(test_file, encoding="utf-8") as file_object:
            result = custat.StatOutputParser().parse(file_object)

        expected_result = {
            "access_time": "2026-07-18T16:46:27.822277576Z",
            "change_time": "2026-07-18T16:46:27.823277583Z",
            "creation_time": "2026-07-18T16:46:27.822277576Z",
            "file_mode": 0o100644,
            "group_identifier": 0,
            "inode_number": 22,
            "modification_time": "2026-07-18T16:46:27.822277576Z",
            "number_of_blocks": 0,
            "number_of_links": 1,
            "path": "p1/testdir1/xattr1",
            "size": 0,
            "user_identifier": 0,
        }
        self.assertEqual(result, expected_result)


if __name__ == "__main__":
    unittest.main()
