...
# https://github.com/dfirlabs/apfs-specimens/blob/master/generate-specimens-unicode-macos.sh

EXIT_FAILURE=1;
EXIT_SUCCESS=0;

FSAPFSINFO="fsapfsinfo";
...
for FILE in `${FSAPFSINFO} -o $(( 40 * 512 )) -H ${SOURCE} | sort | grep '/testdir1/unicode_'`;
do
	# Strip the volume path segment
	...
	OUTPUT=`${FSAPFSINFO} -o $(( 40 * 512 )) -F ${LOOKUP_PATH} ${SOURCE} | grep 'Path' | sed 's/^.*: //'`;
